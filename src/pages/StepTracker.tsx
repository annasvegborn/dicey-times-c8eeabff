...
const StepTracker = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-indigo-900 via-purple-900 to-indigo-800 pb-16">
      ...
      {/* Daily progress */}
      <div className="p-4">
        <div className="bg-stone-100 rounded-lg p-4 border-2 border-amber-700 mb-4">
          <div className="flex justify-between mb-1">
            <span className="font-medium">Daily Goal</span>
            <span className="text-amber-700">{steps.toLocaleString()} / {dailyGoal.toLocaleString()}</span>
          </div>
          ...
          {steps >= dailyGoal && (
            <div className="mt-2 flex items-center gap-2 text-green-600 text-sm">
              <Award size={16} />
              <span>Daily goal completed! +25 XP awarded</span>
            </div>
          )}
        </div>
        
        {/* Map view */}
        <div className="bg-stone-100 rounded-lg overflow-hidden border-2 border-amber-700">
          <div className="relative h-60 bg-[url('https://img.freepik.com/free-photo/old-paper-texture_1194-6201.jpg')] bg-cover">
            {/* Path */}
            <div className="absolute top-1/2 left-0 right-0 h-2 bg-amber-800/30 z-0"></div>
          ...
          <div className="p-4">
            {nextDest ? (
              <>
                <h3 className="font-medium text-amber-800">Next Destination: {nextDest.destination.name}</h3>
                ...
                <div className="mt-3 text-center text-sm">
                  <span className="text-amber-700 font-medium">{nextDest.stepsLeft.toLocaleString()} steps left</span> to reach {nextDest.destination.name}
                </div>
              </>
            ) : (
              <div className="text-center py-3">
                <h3 className="font-medium text-green-700 mb-2">All destinations reached!</h3>
                <p className="text-sm text-gray-600">You've reached all destinations on this path. New areas will unlock tomorrow!</p>
              </div>
            )}
          </div>
        </div>
        ...
        {/* Rewards section */}
        <div className="bg-stone-100 rounded-lg p-4 border-2 border-amber-700 mt-4">
          <h3 className="font-medium text-amber-800 mb-2">Step Rewards</h3>
          <div className="space-y-2">
            <div className="flex justify-between items-center p-2 bg-stone-200 rounded-lg">
              <span>1,000 steps</span>
              <span className={`px-2 py-1 rounded text-xs font-medium ${
                steps >= 1000 ? "bg-green-600 text-white" : "bg-stone-300 text-stone-600"
              }`}>
            ...
            <div className="flex justify-between items-center p-2 bg-stone-200 rounded-lg">
              ...
              <span className={`px-2 py-1 rounded text-xs font-medium ${
                steps >= 5000 ? "bg-green-600 text-white" : "bg-stone-300 text-stone-600"
              }`}>
            ...
            <div className="flex justify-between items-center p-2 bg-stone-200 rounded-lg">
              ...
              <span className={`px-2 py-1 rounded text-xs font-medium ${
                steps >= 10000 ? "bg-green-600 text-white" : "bg-stone-300 text-stone-600"
              }`}>
